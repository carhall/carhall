module CheyouhuiHelper
    ...
    def response_for_click(tag)
    	...
    	when "xiche"
    		article=[generate_article("洗车","洗车","weixin/secondhand_appraise.png","#{host}/weixin/cheyouhui/cleanings?token=#{token}")]  		
    	when "buycar"
    	...
    	when 'baoyang'
    		article=[generate_article("保养","查看等多保养信息","weixin/buying_advice.png","#{host}/weixin/cheyouhui/mendings?token=#{token}")]
    	end
